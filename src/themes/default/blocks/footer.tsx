    ...
    >
      <div className="container space-y-8 overflow-x-hidden">
        <div className="grid min-w-0 gap-12 md:grid-cols-5">
          ...
          <div className="col-span-3 grid min-w-0 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                ...
                <div className="flex min-w-0 flex-wrap gap-4 sm:flex-col">
                  {item.children?.map((subItem, iidx) => (
                    <Link
                      ...
                      aria-label={
                        subItem.title || subItem.image?.alt || 'Footer link'
                      }
                      className={
                        subItem.image?.src
                          ? 'inline-flex items-center hover:opacity-90 duration-150'
                          : 'text-muted-foreground hover:text-primary block break-words duration-150'
                      }
                    >
                      {subItem.image?.src ? (
                        <img
                          ...
                          loading="lazy"
                          className={`max-w-full ${subItem.image.className || ''}`.trim()}
                        />
                      ) : (
